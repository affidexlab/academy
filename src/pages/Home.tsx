...
function TeamSlider() {
  ...
  const go = (idx: number) => {
    setCurrent((idx + team.length) % team.length);
    setIsAutoPlaying(false);
    if (timerRef.current) clearInterval(timerRef.current);
  };

  return (
    <section className="py-16 md:py-28" style={{ background: "var(--navy)", overflow: "hidden" }}>
      <div className="container flex flex-col gap-10">
        {/* Header */}
        <div className="flex items-end justify-between mb-10 md:mb-16">
          <div>
            <div className="label mb-4" style={{ color: "var(--gold)" }}>Leadership Team</div>
            <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(2.2rem,4vw,3.4rem)", fontWeight: 700, lineHeight: 1.1, color: "#FFFFFF" }}>
              The people behind<br />
              <span className="italic" style={{ color: "var(--gold-2)" }}>the mission.</span>
            </h2>
          </div>
          {/* Arrow controls */}
        ...
        {/* Slides — desktop: show all 4, mobile: show active */}
        <div className="hidden md:grid md:grid-cols-4 gap-5">
          {team.map((m, i) => (
            <div key={i}
              ...
              style={{
                border: i === current
                  ? "1.5px solid rgba(200,146,42,0.6)"
                  : "1px solid rgba(255,255,255,0.08)",
                background: i === current
                  ? "rgba(200,146,42,0.08)"
                  : "rgba(255,255,255,0.04)",
                transform: i === current ? "translateY(-4px)" : "none",
                boxShadow: i === current ? "0 20px 50px rgba(200,146,42,0.15)" : "none",
              }}
              onClick={() => go(i)}>
              {/* Photo */}
              <div className="relative overflow-hidden" style={{ height: "220px", background: "rgba(255,255,255,0.05)" }}>
                {m.photo ? (
                  <img src={m.photo} alt={m.name}
                    className="w-full h-full object-cover object-top"
                    style={{ filter: i !== current ? "grayscale(40%) brightness(0.75)" : "none", transition: "filter 0.3s" }} />
                ) : (
                  <div className="w-full h-full flex items-center justify-center"
                    style={{ background: "linear-gradient(135deg,rgba(11,28,46,0.8),rgba(26,53,88,0.8))" }}>
                    <span className="text-4xl font-black" style={{ color: "var(--gold-2)", fontFamily: "'Cormorant Garamond',serif", opacity: 0.8 }}>
                      {m.initials}
                    </span>
                  </div>
                )}
              ...
              <div className="p-5 flex flex-col gap-2">
                <h4 className="text-[14px] font-bold leading-tight text-white mb-1">{m.name}</h4>
                <p className="text-[12px] font-semibold" style={{ color: "var(--gold-2)" }}>{m.role}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="md:hidden">
          <div className="rounded-2xl overflow-hidden" style={{ border: "1.5px solid rgba(200,146,42,0.5)" }}>
            <div className="relative" style={{ height: "280px", background: "rgba(255,255,255,0.05)" }}>
              {team[current].photo ? (
                <img src={team[current].photo} alt={team[current].name}
                  className="w-full h-full object-cover object-top" />
              ) : (
                <div className="w-full h-full flex items-center justify-center"
                  style={{ background: "linear-gradient(135deg,rgba(11,28,46,0.8),rgba(26,53,88,0.8))" }}>
                  <span className="text-6xl font-black" style={{ color: "var(--gold-2)", fontFamily: "'Cormorant Garamond',serif" }}>
            ...
            <div className="p-6">
              ...
              <p className="text-[13px] font-semibold" style={{ color: "var(--gold-2)" }}>{team[current].role}</p>
            </div>
          </div>
          {/* Dots + arrows */}
          <div className="flex items-center justify-center gap-4 mt-5 md:mt-6">
            <button onClick={() => go(current - 1)} style={{ color: "rgba(255,255,255,0.5)" }}>
              <ChevronLeft size={20} />
            </button>
...
export default function AcademyHome() {
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section style={{ background: "var(--navy)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="container py-10 md:py-14">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px" style={{ background: "rgba(255,255,255,0.07)" }}>
            {stats.map((s, i) => (
              <div key={i} className="text-center px-4 py-8 md:px-8 md:py-10" style={{ background: "var(--navy)" }}>
      ...
      {/* ═══════════════ PARTNER TICKER ══════════════════════════════ */}
      <section style={{ background: "var(--navy-2)", borderBottom: "1px solid rgba(255,255,255,0.06)" }} className="py-8 md:py-10 overflow-hidden flex flex-col gap-5">
        <div className="flex justify-center mb-5 md:mb-7">
          <p className="label" style={{ color: "var(--gold)" }}>Confirmed Affiliations & Partners</p>
        </div>
        <div className="relative">
          ...
          <div className="ticker-inner">
            {partners.map((p, i) => (
              <div key={i} className="flex items-center gap-3 md:gap-4 mx-2 md:mx-3 flex-shrink-0 rounded-2xl px-5 py-3 md:px-6 md:py-4"
      ...
      <section className="py-16 md:py-28" style={{ background: "#FFFFFF" }}>
        <div className="container flex flex-col gap-10">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {whyUs.map((c, i) => (
              <div key={i} className="card rounded-2xl border p-6 md:p-8 flex flex-col gap-2" style={{ borderColor: "var(--border)" }}>
                <div className="mb-4 md:mb-5 inline-flex rounded-2xl p-3 md:p-3.5 w-fit" style={{ background: "rgba(11,28,46,0.06)" }}>
                  <span style={{ color: "var(--navy)" }}>{c.icon}</span>
                </div>
                <h3 className="text-[15px] font-bold mb-1.5 md:mb-2.5" style={{ color: "var(--navy)" }}>{c.title}</h3>
      ...
      <section className="py-16 md:py-28" style={{ background: "var(--cream)", borderTop: "1px solid var(--border)" }}>
        <div className="container flex flex-col gap-10">
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {programmes.map((p, i) => (
              <div key={i} className="card rounded-2xl border bg-white p-5 md:p-7 flex flex-col gap-2" style={{ borderColor: "var(--border)" }}>
                ...
                <h3 className="text-[14.5px] font-bold leading-snug mb-1.5 md:mb-2" style={{ color: "var(--navy)" }}>{p.title}</h3>
                <div className="flex items-center gap-2 text-[12px] mb-2.5 md:mb-3" style={{ color: "var(--text-light)" }}>
                  <BookOpen size={12} /> {p.weeks} weeks · {p.level}
                </div>
                <p className="text-[13px] leading-relaxed mb-4 md:mb-5" style={{ color: "var(--text-mid)" }}>{p.desc}</p>
      ...
      {/* ═══════════════ AFFILIATIONS GRID ═══════════════════════════ */}
      <section className="py-16 md:py-28" style={{ background: "#FFFFFF", borderTop: "1px solid var(--border)" }}>
        <div className="container flex flex-col gap-10">
          <div className="mb-12 md:mb-20">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={i} className="card rounded-2xl border p-5 flex flex-col gap-2" style={{ borderColor: "var(--border)" }}>
        ...
        <div className="container flex flex-col gap-10">
          ...
          <div className="grid gap-6 md:grid-cols-3 max-w-[900px] mx-auto">
            {[
              ...
              <div key={i} className="relative rounded-2xl border bg-white p-9 flex flex-col gap-2" style={{ borderColor: "var(--border)" }}>
      ...
      <section className="relative overflow-hidden noise-overlay py-28"
        ...
        <div className="container relative z-10 text-center text-white flex flex-col gap-5">
